"""
포트폴리오(김민도)_v4.pptx - 박지명 스타일 기반 프리미엄 디자인
Design: "Park Ji-myung Layout & Modern Premium Accents"
- 레이아웃: 상단 헤더 배너 + 좌측 5탭 세로 네비게이션 사이드바 (박지명 스타일)
- 표지 & 엔딩: 심플 딥 슬레이트 테크 미니멀 디자인
- 목차: 가로형 로우 카드(Row Card) 형태의 정돈된 레이아웃
- 본문 콘텐츠 슬라이드:
  - 좌측 액티브 탭 카드화 (화이트 둥근 카드 + 컬러 배지 노드)
  - 우측 이미지 프레임 카드 (스크린샷 디바이스 목업 느낌의 배경 카드 & 이탤릭 설명)
  - 텍스트 전용 슬라이드: 5개 이상 항목일 경우 자동 2컬럼 분할 배치 및 중앙 세로선
- 간지 (Divider): 세련된 좌우 스플릿 레이아웃 + 프로젝트별 테크 뱃지 알약 배치
"""
import os
...
def draw_tech_pills(slide, start_x, start_y, pills_list, fill_color, text_color):
    """Draws tech stack pills on the Divider screen"""
    curr_x = start_x
    curr_y = start_y
    for pill in pills_list:
        ...
        # Pill Shape (White fill with accent color line and text)
        p_shp = rrect(slide, curr_x, curr_y, pill_w, 0.35, BG_LIGHT, text_color)
        tf = p_shp.text_frame
        tf.word_wrap = False
...
# ─── PARK JI-MYUNG HEADER BANNER ───
def add_header_banner(slide, project_label, accent_color):
    # Header Background Card
    rect(slide, 0, 0, 13.333, 0.85, GRAY50, None)
    
    # Left accent vertical strip (thinner & cleaner: 0.1)
    rect(slide, 0, 0, 0.1, 0.85, accent_color)
    
    # PORTFOLIO text with space simulation
    t1 = tx(slide, 0.3, 0.1, 2.5, 0.35)
    p1 = t1.text_frame.paragraphs[0]
    rn(p1, "P O R T F O L I O", 10.5, True, accent_color)
    
    # Sub title '포트폴리오'
    t2 = tx(slide, 0.3, 0.45, 2.5, 0.3)
    p2 = t2.text_frame.paragraphs[0]
    rn(p2, "백엔드 개발자 포트폴리오", 8.5, False, GRAY400)
    
    # Project Label text on the right
    t3 = tx(slide, 3.0, 0.15, 10.0, 0.55)
    p3 = t3.text_frame.paragraphs[0]; p3.alignment = PP_ALIGN.RIGHT
    rn(p3, project_label, 17, True, GRAY900)

# ─── SIDEBAR TAB NAVIGATION (Park Ji-myung style) ───
TABS_CONTENT = ["01 | 개요 · 기획의도", "02 | 핵심 구현", "03 | 데이터 구조", "04 | 트러블슈팅", "05 | 회고"]

def add_sidebar_nav(slide, part_label, active_tab_idx, accent_color):
    # Sidebar Background shape
    rect(slide, 0, 0.85, 2.7, 6.65, GRAY50, None)
    
    # Part Label Header
    pl = tx(slide, 0.2, 0.98, 2.3, 0.45)
    p0 = pl.text_frame.paragraphs[0]
    rn(p0, part_label, 12, True, accent_color)
    ...
    for idx, tab_name in enumerate(TABS_CONTENT):
        ...
        if is_active:
            # Draw beautiful rounded active card shape (white card with thin shadow/border)
            rrect(slide, 0.15, top_y - 0.05, 2.45, 0.88, BG_LIGHT, GRAY200)
            
            # Left accent glowing strip on the active card
            rect(slide, 0.15, top_y - 0.05, 0.08, 0.88, accent_color)
            
            # Tab text box
            tt = tx(slide, 0.33, top_y + 0.05, 2.2, 0.8)
            tt.text_frame.word_wrap = True
            tp = tt.text_frame.paragraphs[0]
            rn(tp, num + " | ", 10, True, accent_color)
            rn(tp, name, 10, True, GRAY900)
        else:
            # Inactive Tab Text
            tt = tx(slide, 0.33, top_y + 0.05, 2.2, 0.8)
            tt.text_frame.word_wrap = True
            tp = tt.text_frame.paragraphs[0]
            rn(tp, num + " | ", 10, False, GRAY400)
            rn(tp, name, 10, False, GRAY400)
...
def parse_bullet_text(tf, items, accent, is_first_p=True):
    for item in items:
        ...
        # Prepend colored square bullet matching active project neon color
        rn(p, "▸ ", 11, True, accent)
        ...
        if "**" in item:
            ...
            for pi, part in enumerate(parts):
                if pi % 2 == 1:
                    rn(p, part, 11, True, GRAY900) # Bold keyword
                else:
                    rn(p, part, 11, color=GRAY700) # Regular body
        else:
            rn(p, item, 11, color=GRAY700)

# ─── CONTENT AREA BUILDER ───
def build_content_area(slide, title, items, left=2.9, top=1.0, width=10.2, has_img=False, accent=P1_MAIN):
    # Category / Title accent vertical stripe
    rect(slide, left, top + 0.05, 0.06, 0.42, accent)
    
    # Title Text Box
    ttb = tx(slide, left + 0.15, top, width - 0.15, 0.55)
    tp_t = ttb.text_frame.paragraphs[0]
    rn(tp_t, title, 17, True, GRAY900)
    
    # Content Columns
    if has_img:
        # Left Text area (5.1 inches width)
        ct = tx(slide, left, top + 0.62, 5.1, 4.9)
        tf = ct.text_frame; tf.word_wrap = True
        tf.margin_left = 0; tf.margin_top = 0
        parse_bullet_text(tf, items, accent, is_first_p=True)
    else:
        # Text Only area (10.2 inches width): Split automatically if items >= 5
        if len(items) >= 5:
            mid = (len(items) + 1) // 2
            left_items = items[:mid]
            right_items = items[mid:]
            
            # Left Column (4.8 inches wide)
            ct_l = tx(slide, left, top + 0.62, 4.8, 4.9)
            ...
            rect(slide, left + 5.0, top + 0.7, 0.015, 4.5, GRAY200)
            
            # Right Column (4.8 inches wide)
            ct_r = tx(slide, left + 5.2, top + 0.62, 4.8, 4.9)
            ...
            # Single wide column
            ct = tx(slide, left, top + 0.62, width, 4.9)
            tf = ct.text_frame; tf.word_wrap = True
            tf.margin_left = 0; tf.margin_top = 0
            parse_bullet_text(tf, items, accent, is_first_p=True)
...
def add_footer(slide, active_page, accent):
    # Separator Line
    rect(slide, 2.9, 6.8, 10.2, 0.015, GRAY200)
    
    # Left Text
    lt = tx(slide, 2.9, 6.86, 6.0, 0.3)
    lp = lt.text_frame.paragraphs[0]
    rn(lp, "MIN DO KIM  ·  BACKEND DEVELOPER PORTFOLIO", 8, True, GRAY400)
    
    # Page Indicator (Neon accent color)
    pt = tx(slide, 12.0, 6.86, 1.1, 0.3)
    pp = pt.text_frame.paragraphs[0]; pp.alignment = PP_ALIGN.RIGHT
    rn(pp, active_page, 9.5, True, accent)

# ─── MAIN CONTENT SLIDE COMPILER ───
def content_slide(project_label, part_label, active_tab_idx, title, items, accent, img_path=None, img_caption=None, active_page=""):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    bg(sl, BG_LIGHT)
    
    add_header_banner(sl, project_label, accent)
    add_sidebar_nav(sl, part_label, active_tab_idx, accent)
    
    # Separator Line between sidebar and content area
    rect(sl, 2.7, 0.85, 0.015, 6.65, GRAY200)
    
    has_img = img_path and os.path.exists(img_path)
    
    # Build Content Text Areas
    build_content_area(sl, title, items, left=2.9, top=1.05, width=10.2, has_img=has_img, accent=accent)
    
    # Picture Card frame (Right side)
    if has_img:
        # Light Slate device mockup frame card
        rrect(sl, 8.0, 1.5, 5.0, 5.15, GRAY50, GRAY200)
        # Picture centering inside card
        pic(sl, img_path, 8.15, 1.65, 4.7, 3.9)
        # Caption below image
        if img_caption:
            tb_cap = tx(sl, 8.15, 5.65, 4.7, 0.35)
...
# ─── DEV SPLIT SCREEN DIVIDER (간지) SLIDE ───
def divider(part_num, title, subtitle, accent, accent_lite, pills_list=None):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    bg(sl, DARK)
    
    # Right Side Split Screen background
    rect(sl, 4.6, 0, 8.733, 7.5, GRAY50, None)
    
    # Split vertical divider neon accent strip
    rect(sl, 4.6, 0, 0.08, 7.5, accent)
    
    # Left (Dark) Side Content
    # Part indicator text
    ...
    rn(np, f"0{part_num}", 100, True, BG_LIGHT)
    
    # Category tag below number
    ...
    tp = tt.text_frame.paragraphs[0]
    rn(tp, title, 36, True, GRAY900)
    
    # Subtitle
    st = tx(sl, 5.2, 3.0, 7.2, 1.0)
    st.text_frame.word_wrap = True
    sp = st.text_frame.paragraphs[0]
    rn(sp, subtitle, 14, color=GRAY700)
    
    # Tech Stack Divider line
    rect(sl, 5.2, 4.3, 7.2, 0.015, GRAY200)
    
    # Tech Badge Pills (using white pill backgrounds with neon border/text)
    if pills_list:
        draw_tech_pills(sl, 5.2, 4.7, pills_list, accent_lite, accent)
        
    return sl


# ══════════════════════════════════════════════════════════════════
# SLIDE 1: COVER (Modern Minimal Dark)
# ══════════════════════════════════════════════════════════════════
sl = prs.slides.add_slide(prs.slide_layouts[6])
bg(sl, DARK)

# 4 Project Neon Accent Stripes on the right
rect(sl, 9.2, 0, 0.8, 7.5, P1_MAIN)
rect(sl, 10.1, 0, 0.8, 7.5, P2_MAIN)
rect(sl, 11.0, 0, 0.8, 7.5, P3_MAIN)
rect(sl, 11.9, 0, 0.8, 7.5, P4_MAIN)

# Left Side Decorative Vertical neon bar
...
rn(p2, "KIM MIN DO", 52, True, BG_LIGHT)
...
# ══════════════════════════════════════════════════════════════════
# SLIDE 2: PROFILE & SKILLS (Dashboard Double Cards)
# ══════════════════════════════════════════════════════════════════
sl = prs.slides.add_slide(prs.slide_layouts[6])
bg(sl, BG_LIGHT)

# Top Header Card Pill (aligned with slide borders)
rect(sl, 0, 0, 13.333, 0.85, GRAY50, None)
rect(sl, 0, 0, 0.1, 0.85, P1_MAIN)
th_t = tx(sl, 0.3, 0.22, 12.0, 0.5)
th_p = th_t.text_frame.paragraphs[0]
rn(th_p, "ABOUT ME  &  TECHNICAL SKILLS", 18, True, GRAY900)

# Left Card: Profile
rrect(sl, 0.5, 1.1, 6.0, 5.6, GRAY50, GRAY200)
lc_t = tx(sl, 0.75, 1.3, 5.5, 5.2)
...
for label, val in [
    ...
    pl = ltf.add_paragraph(); pl.space_before = Pt(6)
    rn(pl, f"▸ {label}", 12, True, GRAY900)
    pv = ltf.add_paragraph(); pv.space_after = Pt(4)
    rn(pv, val, 11, color=GRAY700)

# Right Card: Technical Skills
rrect(sl, 6.8, 1.1, 6.0, 5.6, GRAY50, GRAY200)
rc_t = tx(sl, 7.05, 1.3, 5.5, 5.2)
...
for cat, itms in [
    ...
    pc = rtf.add_paragraph(); pc.space_before = Pt(8)
    rn(pc, f"▸ {cat}", 12, True, GRAY900)
    pi = rtf.add_paragraph(); pi.space_after = Pt(4)
    rn(pi, itms, 11, color=GRAY700)

# Footer
rect(sl, 0.5, 6.9, 12.333, 0.015, GRAY200)
lt = tx(sl, 0.5, 6.96, 5.0, 0.3)
rn(lt.text_frame.paragraphs[0], "MIN DO KIM  ·  ABOUT ME", 8, True, GRAY400)
pt = tx(sl, 12.0, 6.96, 0.833, 0.3)
rn(pt.text_frame.paragraphs[0], "02 / 35", 9.5, True, P1_MAIN)


# ══════════════════════════════════════════════════════════════════
# SLIDE 3: TABLE OF CONTENTS (Row Card Deck)
# ══════════════════════════════════════════════════════════════════
sl = prs.slides.add_slide(prs.slide_layouts[6])
bg(sl, BG_LIGHT)
...
rect(sl, 0, 0, 13.333, 0.85, GRAY50, None)
rect(sl, 0, 0, 0.1, 0.85, P1_MAIN)
toc_t = tx(sl, 0.3, 0.22, 12.0, 0.5)
toc_p = toc_t.text_frame.paragraphs[0]
rn(toc_p, "PORTFOLIO INDEX  ·  목차", 18, True, GRAY900)
...
for i, (part, name, desc, clr) in enumerate(projects):
    row_y = 1.15 + i * 1.35
    # Row Card Background shape
    rrect(sl, 0.5, row_y, 12.333, 1.15, GRAY50, GRAY200)
    
    # Left accent vertical bar on card
    rect(sl, 0.5, row_y, 0.15, 1.15, clr)
    
    # Row Text Box
    row_tb = tx(sl, 0.8, row_y + 0.12, 11.8, 1.0)
    rtf = row_tb.text_frame; rtf.word_wrap = True
    
    p = rtf.paragraphs[0]
    rn(p, part + "   ", 12, True, clr)
    rn(p, name, 15, True, GRAY900)
    
    p2 = rtf.add_paragraph(); p2.space_before = Pt(4)
    rn(p2, desc, 10.5, False, GRAY700)
...
content_slide(GM_L, GM_PART, 0, "개요 · 기획의도", [
...
content_slide(GM_L, GM_PART, 1, "핵심 구현 사항 — 요약", [
...
content_slide(GM_L, GM_PART, 1, "Elasticsearch 기반 검색 성능 최적화", [
...
content_slide(GM_L, GM_PART, 1, "Redis 다중 활용 — 캐싱 · 세션 · Rate Limiter", [
...
content_slide(GM_L, GM_PART, 1, "Kafka 비동기 이벤트 격리 — 결제 완료 처리", [
...
content_slide(GM_L, GM_PART, 2, "배송 테이블 통합 & 외부 API 호출 최소화", [
...
content_slide(GM_L, GM_PART, 3, "React 부분 렌더링 & 뒤로가기 캐시 문제", [
...
content_slide(GM_L, GM_PART, 3, "공동구매 자동 대기열 & 동시성 제어", [
    "**공동구매 자동 참여 전환 대기열**",
...
content_slide(GM_L, GM_PART, 3, "주문 취소 동시성 & 환불 멱등성 보장", [
...
content_slide(GM_L, GM_PART, 3, "GitHub Actions CI/CD & AWS EC2 배포", [
...
content_slide(GM_L, GM_PART, 3, "매출 통계 N+1 최적화 & 대량 쿠폰 발급", [
...
content_slide(GM_L, GM_PART, 4, "프로젝트 완수 후기 · 회고", [
    "**배포 및 보안 경험**",
...
content_slide(JS_L, JS_PART, 0, "개요 · 기획의도", [
...
content_slide(JS_L, JS_PART, 1, "핵심 구현 사항", [
...
content_slide(JS_L, JS_PART, 2, "시스템 아키텍처 & DB 설계", [
...
content_slide(JS_L, JS_PART, 3, "K6 부하 테스트 & 성능 개선", [
...
content_slide(JS_L, JS_PART, 4, "프로젝트 후기 · 회고", [
    "**동시성 트레이드 오프**",
...
content_slide(DT_L, DT_PART, 0, "개요 · 기획의도", [
...
content_slide(DT_L, DT_PART, 1, "핵심 구현 사항", [
...
content_slide(DT_L, DT_PART, 2, "무결성 & 관계 모델링", [
...
content_slide(DT_L, DT_PART, 3, "비정규화 성능 최적화 & Soft Delete 대응", [
...
content_slide(DT_L, DT_PART, 4, "프로젝트 후기 · 회고", [
    "**순수 자바 웹 아키텍처 이해**",
...
content_slide(ST_L, ST_PART, 0, "개요 · 기획의도", [
...
content_slide(ST_L, ST_PART, 1, "핵심 구현 사항", [
...
content_slide(ST_L, ST_PART, 2, "의존성 트리 & 관계 재정의", [
...
content_slide(ST_L, ST_PART, 3, "제약 조건 최적화 & 트리거 설계", [
    "**CHECK 제약조건으로 트리거 대체**",
...
content_slide(ST_L, ST_PART, 4, "프로젝트 후기 · 회고", [
    "**데이터 설계의 주춧돌**",
...
# SAVE PRESENTATION
# ══════════════════════════════════════════════════════════════════
out_path = r"C:\Users\a\Desktop\쇼핑몰 프로젝트 자료모음\6_자소서, 포폴 1차 완성본_260525\포트폴리오(김민도)_v4.pptx"
prs.save(out_path)
