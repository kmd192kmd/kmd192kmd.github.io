"""
포트폴리오(김민도)_v8.pptx - 완전히 혁신된 맞춤형 레이아웃 디자인 (반복적인 템플릿 제거)
Design Concept: "Dynamic Custom Grid & Contextual Layouts"
- 반복성 탈피: 모든 콘텐츠 슬라이드가 동일한 레이아웃을 사용하던 방식에서 탈피하여 슬라이드 성격에 맞는 전용 레이아웃 함수 설계
- 1. 개요 (Overview): 좌측 'Project Spec Card' (기간, 인원, 역할, 기술) + 우측 '메인 스크린 샷' 및 기획의도 요약 배치
- 2. 핵심 구현 (Features Grid): 기본 글머리 기호 나열에서 탈피하여 3열 2행 혹은 4열 2행의 '구현 카드 그리드'로 인포그래픽화
- 3. 상세 기술 (Tech Detail): 비대칭 텍스트-이미지 분할 혹은 이미지 히어로 배치
- 4. 트러블슈팅 (Troubleshooting): 좌측 세로 적층형 'PROBLEM & CONTEXT' / 'SOLUTION & RESOLUTION' 카드덱 + 우측 스크린샷 카드 배치
- 5. 회고 (Retrospective): 좌측 다크 테마의 'Developer's Quote' 블록 + 우측 성찰 요점 카드 레이아웃
- 디자인 디테일: 상단 25% 헤더 밴드를 걷어내고, 여백과 얇은 세로 라인, 깔끔한 폰트 위계의 '미니멀 탑 헤더'로 공간 확보
"""
import os
...
def add_clean_header(slide, active_proj_idx, section_name, title, accent):
    ...
    # Project title & category on the right
...
# ══════════════════════════════════════════════════════════════════
# DYNAMIC CONTEXTUAL LAYOUT FUNCTIONS
# ══════════════════════════════════════════════════════════════════

# 1. OVERVIEW LAYOUT (Spec card + Image + Description card)
def overview_slide(project_idx, section, title, items, accent, img_path=None, img_caption=None, active_page=""):
    ...
    # Parse items into specs vs description
    specs = []
    desc_items = []
    for item in items:
    ...
    rect(sl, 0.8, 1.6, 3.8, 0.08, accent) # top accent line
    ...
    if has_img:
        ...
        if img_caption:
            ...
            tf_c = tb_cap.text_frame; tf_c.margin_left = tf_c.margin_top = tf_c.margin_bottom = tf_c.margin_right = 0
        ...
        for di in desc_items:
            p = tf_d.paragraphs[0] if is_first_d else tf_d.add_paragraph()
            is_first_d = False
            p.space_after = Pt(6)
            ...
            if "**" in di:
                ...
                for pi, part in enumerate(parts):
                    if pi % 2 == 1:
                        rn(p, part, 10.5, True, GRAY900)
                    else:
                        rn(p, part, 10.5, color=GRAY700)
            else:
                rn(p, di, 10.5, color=GRAY700)
    else:
        # Description text only, full width
        tb_desc = tx(sl, 4.9, 1.6, 7.633, 5.0)
        tf_d = tb_desc.text_frame; tf_d.word_wrap = True
        tf_d.margin_left = tf_d.margin_top = tf_d.margin_bottom = tf_d.margin_right = 0
        parse_bullet_text(tf_d, desc_items, accent, is_first_p=True)

# 2. CORE FEATURES GRID LAYOUT (Pills summary mapped to dynamic cards)
def features_slide(project_idx, section, title, items, accent, active_page=""):
    ...
    for idx, item in enumerate(items):
        ...
        rect(sl, x, y, w_card, 0.06, accent) # top color border
...
# 3. TECHNICAL DETAIL LAYOUT (Asymmetric text + image frame)
def technical_slide(project_idx, section, title, items, accent, img_path=None, img_caption=None, active_page=""):
    ...
    if has_img:
        ...
        # Right side: Large Image Frame Card
        ...
        # Split into columns automatically if item count >= 5
        if len(items) >= 5:
            mid = (len(items) + 1) // 2
...
# 4. TROUBLESHOOTING LAYOUT (Problem/Cause card + Solution/Result card + optional Image)
...
# 5. RETROSPECTIVE LAYOUT (Quote Card + Horizontal Lessons Stack)
RETRO_QUOTES = {
...
def retrospective_slide(project_idx, section, title, items, accent, active_page=""):
    ...
    # Left Card (Developer Quote)
    rrect(sl, 0.8, 1.6, 4.8, 5.0, DARK, None)
    rect(sl, 0.8, 1.6, 0.12, 5.0, accent) # Left vertical border
    ...
    quote_text = RETRO_QUOTES.get(project_idx, "배운 점과 고찰을 바탕으로 더 성장하는 백엔드 개발자가 되겠습니다.")
    rn(p_q2, f'"{quote_text}"', 14, False, WHITE, italic=True)
    
    # Right Column: Parse lessons
    lessons = []
    curr_title = ""
    curr_desc = []
    ...
    for item in items:
        if item.startswith("**") and item.endswith("**"):
            ...
            curr_title = item.replace("**", "")
            curr_desc = []
        else:
            if not curr_title:
                curr_title = "Lesson Learned"
            curr_desc.append(item)
            
    if curr_title:
        lessons.append((curr_title, " ".join(curr_desc)))
        
    # Draw maximum 3 lessons as stacked cards
    ...
    for idx, (ltitle, ldesc) in enumerate(lessons[:3]):
        ...
        rect(sl, 6.0, y, 6.5, 0.06, accent) # top border
...
# ══════════════════════════════════════════════════════════════════
# SLIDE 2: PROFILE & SKILLS (Clean Dual Cards Layout with Minimal Header)
# ══════════════════════════════════════════════════════════════════
sl = prs.slides.add_slide(prs.slide_layouts[6])
bg(sl, BG_LIGHT)
...
# Left Card: Profile
rrect(sl, 0.8, 1.6, 5.6, 5.0, GRAY50, GRAY200)
rect(sl, 0.8, 1.6, 5.6, 0.08, P1_MAIN) # top accent line
...
rrect(sl, 6.9, 1.6, 5.6, 5.0, GRAY50, GRAY200)
rect(sl, 6.9, 1.6, 5.6, 0.08, P1_MAIN) # top accent line
...
# ══════════════════════════════════════════════════════════════════
# SLIDE 3: TABLE OF CONTENTS (Modern 4-Column Grid Layout)
# ══════════════════════════════════════════════════════════════════
sl = prs.slides.add_slide(prs.slide_layouts[6])
bg(sl, BG_LIGHT)
...
technical_slide(1, "03  데이터 구조", "시스템 아키텍처 & DB 설계", [
...
troubleshooting_slide(1, "04  트러블슈팅", "K6 부하 테스트 & 성능 개선", [
...
technical_slide(2, "03  데이터 구조", "무결성 & 관계 모델링", [
...
troubleshooting_slide(2, "04  트러블슈팅", "비정규화 성능 최적화 & Soft Delete 대응", [
...
features_slide(3, "02  핵심 구현", "핵심 구현 사항", [
...
technical_slide(3, "03  데이터 구조", "의존성 트리 & 관계 재정의", [
...
troubleshooting_slide(3, "04  트러블슈팅", "제약 조건 최적화 & 트리거 설계", [
    "**CHECK 제약조건으로 트리거 대체**",
...
retrospective_slide(3, "05  회고", "프로젝트 후기 · 회고", [
    "**데이터 설계의 주춧돌**",
...
# SAVE PRESENTATION
# ══════════════════════════════════════════════════════════════════
out_path = r"C:\Users\a\Desktop\쇼핑몰 프로젝트 자료모음\6_자소서, 포폴 1차 완성본_260525\포트폴리오(김민도)_v8.pptx"
prs.save(out_path)
