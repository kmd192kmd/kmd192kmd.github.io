"""
포트폴리오(김민도)_v5.pptx - 완전히 새로운 미니멀리스트 디자인 (No Sidebar, No Header Banner)
Design: "Modern Startup Slide Deck (Minimalist Row Cards & Floating Mockups)"
- 사이드바 & 헤더 배너 완전 배제 (여백과 카드 중심의 트렌디 디자인)
- 배경: 매우 부드러운 그레이/블루 틴트 (slate-50)
- 카드: 흰색 둥근 모서리 카드 (fill White, border slate-200)
- 텍스트: 기존 글머리 기호를 없애고 넘버링 인덱스 (01, 02) 적용
- 본문 2컬럼 배치 시 카드를 독립적으로 2개 배치 (카드 스플릿)
- 간지 (Divider): 프로젝트별 시그니처 단색 솔리드 배경 + 화이트 뱃지 알약 배치
"""
import os
...
def draw_tech_pills(slide, start_x, start_y, pills_list, fill_color, text_color):
    ...
    for pill in pills_list:
        ...
        # Transparent pill card on colored background, white border & text
        p_shp = rrect(slide, curr_x, curr_y, pill_w, 0.35, None, BG_LIGHT)
        tf = p_shp.text_frame
        tf.word_wrap = False
        ...
        rn(p, pill, 9.5, True, BG_LIGHT)
        
        curr_x += pill_w + 0.12

# ─── PARSE BULLET WITH INLINE MARKERS & CUSTOM NUMBER INDEX ───
def parse_bullet_text(tf, items, accent):
    for idx, item in enumerate(items):
        p = tf.paragraphs[0] if idx == 0 else tf.add_paragraph()
        p.space_after = Pt(8); p.space_before = Pt(4)
        
        # Custom index bullet (e.g., "01 ")
        rn(p, f"0{idx+1}  ", 11, True, accent)
        ...
        if "**" in item:
            ...
            for pi, part in enumerate(parts):
                if pi % 2 == 1:
                    ...
                    rn(p, part, 11, color=GRAY700) # Normal body
        else:
            rn(p, item, 11, color=GRAY700)

# ─── HEADER AREA FOR CONTENT SLIDES ───
def add_slide_header(slide, section_name, title, accent):
    # Section Tag Badge (upper left)
    rrect(slide, 0.6, 0.3, 1.4, 0.35, P1_LITE if accent == P1_MAIN else (P2_LITE if accent == P2_MAIN else (P3_LITE if accent == P3_MAIN else P4_LITE)), None)
    t_tag = tx(slide, 0.6, 0.31, 1.4, 0.3)
    p_tag = t_tag.text_frame.paragraphs[0]; p_tag.alignment = PP_ALIGN.CENTER
    rn(p_tag, section_name, 9, True, accent)
    
    # Title
    t_title = tx(slide, 2.1, 0.22, 7.5, 0.55)
    p_title = t_title.text_frame.paragraphs[0]
    rn(p_title, title, 22, True, GRAY900)
    
    # Right Page status pill (e.g. "Gold Market")
    t_stat = tx(slide, 9.8, 0.3, 2.9, 0.4)
    p_stat = t_stat.text_frame.paragraphs[0]; p_stat.alignment = PP_ALIGN.RIGHT
    rn(p_stat, "BACKEND PORTFOLIO", 9.5, True, GRAY400)

# ─── FLOATING FOOTER ───
def add_footer(slide, active_page, accent):
    # Left Label
    lt = tx(slide, 0.6, 6.95, 6.0, 0.3)
    lp = lt.text_frame.paragraphs[0]
    rn(lp, "MIN DO KIM  ·  BACKEND PORTFOLIO", 8, True, GRAY400)
    
    # Page Indicator (Accent color)
    pt = tx(slide, 10.0, 6.95, 2.7, 0.3)
...
def content_slide(project_idx, section, title, items, accent, img_path=None, img_caption=None, active_page=""):
    ...
    add_slide_header(sl, section, title, accent)
    add_footer(sl, active_page, accent)
    
    has_img = img_path and os.path.exists(img_path)
    
    if has_img:
        # Left Text Card (rounded card at 0.6, width 6.6)
        rrect(sl, 0.6, 1.2, 6.6, 5.2, BG_LIGHT, GRAY200)
        ct = tx(sl, 0.85, 1.4, 6.1, 4.8)
        tf = ct.text_frame; tf.word_wrap = True
        tf.margin_left = 0; tf.margin_top = 0
        parse_bullet_text(tf, items, accent)
        
        # Right Image Frame Card (rounded card at 7.5, width 5.2)
        rrect(sl, 7.5, 1.2, 5.2, 5.2, BG_LIGHT, GRAY200)
        pic(sl, img_path, 7.65, 1.35, 4.9, 4.1)
        if img_caption:
            tb_cap = tx(sl, 7.65, 5.55, 4.9, 0.35)
            p = tb_cap.text_frame.paragraphs[0]; p.alignment = PP_ALIGN.CENTER
            rn(p, img_caption, 9, italic=True, color=GRAY500)
    else:
        # Text Only: If >= 5 items, split layout into two side-by-side cards
        if len(items) >= 5:
            mid = (len(items) + 1) // 2
            ...
            # Left Card
            rrect(sl, 0.6, 1.2, 5.9, 5.2, BG_LIGHT, GRAY200)
            ct_l = tx(sl, 0.85, 1.4, 5.4, 4.8)
            tf_l = ct_l.text_frame; tf_l.word_wrap = True
            tf_l.margin_left = 0; tf_l.margin_top = 0
            parse_bullet_text(tf_l, left_items, accent)
            
            # Right Card
            rrect(sl, 6.8, 1.2, 5.9, 5.2, BG_LIGHT, GRAY200)
            ct_r = tx(sl, 7.05, 1.4, 5.4, 4.8)
            tf_r = ct_r.text_frame; tf_r.word_wrap = True
            tf_r.margin_left = 0; tf_r.margin_top = 0
            parse_bullet_text(tf_r, right_items, accent)
        else:
            # Single wide card
            rrect(sl, 0.6, 1.2, 12.133, 5.2, BG_LIGHT, GRAY200)
            ct = tx(sl, 0.9, 1.45, 11.533, 4.7)
            tf = ct.text_frame; tf.word_wrap = True
            tf.margin_left = 0; tf.margin_top = 0
            parse_bullet_text(tf, items, accent)
...
# ─── BOLD SOLID COLOR DIVIDER (간지) SLIDE ───
def divider(part_num, title, subtitle, accent, accent_lite, pills_list=None):
    sl = prs.slides.add_slide(prs.slide_layouts[6])
    bg(sl, accent) # Solid brand color background
    
    # Large Part number
    pt = tx(sl, 0.8, 1.3, 4.0, 0.4)
    pp = pt.text_frame.paragraphs[0]
    rn(pp, f"PART 0{part_num}.", 14, True, BG_LIGHT)
    
    # Large Index Number
    nt = tx(sl, 0.8, 1.8, 3.5, 1.5)
    np = nt.text_frame.paragraphs[0]
    rn(np, f"0{part_num}", 96, True, BG_LIGHT)
    
    # Project Title
    tt = tx(sl, 0.8, 3.5, 11.7, 1.0)
    tp = tt.text_frame.paragraphs[0]
    rn(tp, title, 44, True, BG_LIGHT)
    
    # Subtitle
    st = tx(sl, 0.8, 4.6, 11.7, 0.8)
    st.text_frame.word_wrap = True
    sp = st.text_frame.paragraphs[0]
    rn(sp, subtitle, 15, color=GRAY300)
    
    # Separator line (White thin)
    rect(sl, 0.8, 5.6, 11.7, 0.015, BG_LIGHT)
    
    # Tech Badge Pills (white outlines)
    if pills_list:
        draw_tech_pills(sl, 0.8, 5.9, pills_list, None, BG_LIGHT)
...
# Page header
rrect(sl, 0.6, 0.3, 1.5, 0.35, P1_LITE, None)
t_tag = tx(sl, 0.6, 0.31, 1.5, 0.3)
rn(t_tag.text_frame.paragraphs[0], "ABOUT ME", 9, True, P1_MAIN)

t_title = tx(sl, 2.2, 0.22, 7.5, 0.55)
rn(t_title.text_frame.paragraphs[0], "PROFILE & TECHNICAL SKILLS", 22, True, GRAY900)

# Left Card: Profile
rrect(sl, 0.6, 1.2, 5.9, 5.2, BG_LIGHT, GRAY200)
lc_t = tx(sl, 0.85, 1.4, 5.4, 4.8)
...
# ══════════════════════════════════════════════════════════════════
# SLIDE 3: TABLE OF CONTENTS (Horizontal 4-Card Index)
# ══════════════════════════════════════════════════════════════════
sl = prs.slides.add_slide(prs.slide_layouts[6])
bg(sl, GRAY50)

# Header
rrect(sl, 0.6, 0.3, 1.4, 0.35, P1_LITE, None)
t_tag = tx(sl, 0.6, 0.31, 1.4, 0.3)
rn(t_tag.text_frame.paragraphs[0], "INDEX", 9, True, P1_MAIN)

t_title = tx(sl, 2.2, 0.22, 7.5, 0.55)
rn(t_title.text_frame.paragraphs[0], "PORTFOLIO INDEX", 22, True, GRAY900)

projects = [
    ("Gold Market (골드 마켓)", "Spring Boot 기반 E-Commerce\nElasticsearch · Redis · Kafka\nAWS EC2 · GitHub Actions CI/CD", P1_MAIN),
    ("Jesiyo (제시요)", "WebSocket 실시간 경매 & 거래\nSTOMP · Redis Pub/Sub 메시징\nK6 부하테스트 · Redisson 분산 락", P2_MAIN),
    ("DeveryTime (데버리타임)", "Servlet/JSP MVC Model 2\nAJAX 비동기 통신 & DB 무결성\n학습 관리 커뮤니티 플랫폼", P3_MAIN),
    ("교육센터 운영 관리 시스템", "Oracle PL/SQL DB 프로젝트\n5단계 테이블 의존성 트리 수립\nProcedure / Trigger 최적화", P4_MAIN),
]

for i, (name, desc, clr) in enumerate(projects):
    card_x = 0.65 + i * 3.05
    # Rounded Card background
    rrect(sl, card_x, 1.4, 2.85, 5.1, BG_LIGHT, GRAY200)
    
    # Colored top accent strip on card
    rect(sl, card_x, 1.4, 2.85, 0.12, clr)
    
    # Text Box inside card
    card_tb = tx(sl, card_x + 0.15, 1.65, 2.55, 4.7)
    ctf = card_tb.text_frame; ctf.word_wrap = True
    
    # Index Number
    cp1 = ctf.paragraphs[0]; cp1.space_after = Pt(12)
    rn(cp1, f"0{i+1}", 26, True, clr)
    
    # Project Name
    cp2 = ctf.add_paragraph(); cp2.space_after = Pt(8)
    rn(cp2, name, 14, True, GRAY900)
    
    # Details
    cp3 = ctf.add_paragraph()
    rn(cp3, desc, 10.5, False, GRAY700)
...
content_slide(0, GM_L, "핵심 구현 사항 — 요약", [
...
content_slide(0, GM_L, "Elasticsearch 기반 검색 성능 최적화", [
...
content_slide(0, GM_L, "Redis 다중 활용 — 캐싱 · 세션 · Rate Limiter", [
...
content_slide(0, GM_L, "Kafka 비동기 이벤트 격리 — 결제 완료 처리", [
...
content_slide(0, GM_L, "배송 테이블 통합 & 외부 API 호출 최소화", [
...
content_slide(0, GM_L, "React 부분 렌더링 & 뒤로가기 캐시 문제", [
...
content_slide(0, GM_L, "공동구매 자동 대기열 & 동시성 제어", [
    "**공동구매 자동 참여 전환 대기열**",
...
content_slide(0, GM_L, "주문 취소 동시성 & 환불 멱등성 보장", [
...
content_slide(0, GM_L, "GitHub Actions CI/CD & AWS EC2 배포", [
...
content_slide(0, GM_L, "매출 통계 N+1 최적화 & 대량 쿠폰 발급", [
...
content_slide(0, GM_L, "프로젝트 완수 후기 · 회고", [
    "**배포 및 보안 경험**",
...
content_slide(1, JS_L, "시스템 아키텍처 & DB 설계", [
...
content_slide(1, JS_L, "K6 부하 테스트 & 성능 개선", [
...
content_slide(1, JS_L, "프로젝트 후기 · 회고", [
    "**동시성 트레이드 오프**",
...
content_slide(2, DT_L, "무결성 & 관계 모델링", [
...
content_slide(2, DT_L, "비정규화 성능 최적화 & Soft Delete 대응", [
...
content_slide(2, DT_L, "프로젝트 후기 · 회고", [
    "**순수 자바 웹 아키텍처 이해**",
...
content_slide(3, ST_L, "의존성 트리 & 관계 재정의", [
...
content_slide(3, ST_L, "제약 조건 최적화 & 트리거 설계", [
    "**CHECK 제약조건으로 트리거 대체**",
...
content_slide(3, ST_L, "프로젝트 후기 · 회고", [
    "**데이터 설계의 주춧돌**",
...
# ══════════════════════════════════════════════════════════════════
# SAVE PRESENTATION (Version 5 - Completely New Style, Safe from v4)
# ══════════════════════════════════════════════════════════════════
out_path = r"C:\Users\a\Desktop\쇼핑몰 프로젝트 자료모음\6_자소서, 포폴 1차 완성본_260525\포트폴리오(김민도)_v5.pptx"
prs.save(out_path)
