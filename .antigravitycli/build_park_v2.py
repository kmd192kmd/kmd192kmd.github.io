"""
포트폴리오(박지명)_v2.pptx - 박지명 국비 지원자 기반 디자인 v2
Theme: "Modern Premium Dark Tech Design (Dark Mode)"
- 배경: 딥 슬레이트 다크 (slate-900)
- 카드: 슬레이트 차콜 카드 (slate-800) + 슬레이트 보더 (slate-700)
- 텍스트: 제목 화이트 + 본문 라이트 그레이 (slate-300) / 슬레이트 그레이 (slate-400)
- 악센트: 제시요 (네온 바이올렛/퍼플) | 커뮤니티 (네온 사이언/테알)
- 구성: v1과 동일하되 시각적인 무드가 180도 다른 사이버펑크/테크 프리미엄 다크 테마
"""
import os
...
DARK         = RGBColor(15, 23, 42)      # Deep Slate (slate-900) - 배경색
CARD_BG      = RGBColor(30, 41, 59)      # Slate Blue-gray (slate-800) - 카드 배경
CARD_BORDER  = RGBColor(51, 65, 85)      # Slate gray (slate-700) - 카드 테두리
WHITE        = RGBColor(255, 255, 255)   # White
GRAY300      = RGBColor(203, 213, 225)   # slate-300
GRAY400      = RGBColor(148, 163, 184)   # slate-400
GRAY500      = RGBColor(100, 116, 139)   # slate-500
GRAY700      = RGBColor(203, 213, 225)   # slate-300 (Dark Mode에선 본문 텍스트가 밝아야 하므로)
GRAY900      = RGBColor(255, 255, 255)   # White (Dark Mode에선 타이틀이 화이트)

# Brand Colors (Neon)
P1_MAIN  = RGBColor(168, 85, 247)    # Purple-500 (제시요)
P1_LINE  = RGBColor(216, 180, 254)   # Purple-300

P2_MAIN  = RGBColor(6, 182, 212)     # Cyan-500 (커뮤니티)
P2_LINE  = RGBColor(165, 243, 252)   # Cyan-300
...
def add_footer(slide, active_page, accent):
    ...
    rn(lp, "PARK JI MYUNG  ·  BACKEND DEVELOPER PORTFOLIO", 8, True, GRAY500)
...
for i, (part, name_en, name_ko, desc, clr) in enumerate(projects):
    x = 0.8 + i * 5.9
    rrect(sl, x, 1.6, 5.6, 5.0, CARD_BG, CARD_BORDER)
...
features_slide(0, "02  핵현 구현", "핵심 구현 사항 (담당: 채팅 파트)", [
...
# SAVE PRESENTATION
# ══════════════════════════════════════════════════════════════════
out_path = r"C:\Users\a\Desktop\쇼핑몰 프로젝트 자료모음\6_자소서, 포폴 1차 완성본_260525\포트폴리오(박지명)_v2.pptx"
prs.save(out_path)
